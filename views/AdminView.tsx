...
import { Phone, Check, Trash2, ArrowRightCircle, Bell, MessageCircle, Database, Share2, X, Info } from 'lucide-react';
...
const AdminView: React.FC<AdminViewProps> = ({ queue, onUpdateStatus, onReset, dbConfig, onSaveDbConfig }) => {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
            ...
            {activeList.map(item => (
                <div key={item.id} className="bg-gradient-to-br from-cyan-400 to-blue-500 text-sky-950 p-6 rounded-[2rem] shadow-xl flex justify-between items-center">
                    <div className="flex items-center gap-6">
                        <span className="text-6xl font-black">{item.number}</span>
                        <div>
                            <div className="font-black text-xl">{item.name}</div>
                            <div className="text-sky-900/60 font-bold text-sm">{item.phone}</div>
                        </div>
                    </div>
